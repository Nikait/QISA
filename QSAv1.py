...
class QSA(tq.QuantumModule):
    class QueryKeyLayer(tq.QuantumModule):
        ...
        def forward(self, x: Tensor, qdev: tq.QuantumDevice) -> Tensor:
            ...
            out = out.view(B, T, -1)  # Reshape to match (B, T, feature_dim)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q, k, v = self.q(x, qdev), self.k(x, qdev), self.v(x, qdev)
        q, k = q.repeat(1, 1, self.head_size), k.repeat(1, 1, self.head_size) # (B, T, head_size)
